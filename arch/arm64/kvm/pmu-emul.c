...
{
	u64 mask = kvm_pmu_implemented_counter_mask(vcpu);
	u64 enabled, disabled;

	__vcpu_sys_reg(vcpu, PMOVSSET_EL0) &= mask;
	__vcpu_sys_reg(vcpu, PMINTENSET_EL1) &= mask;
	__vcpu_sys_reg(vcpu, PMCNTENSET_EL0) &= mask;

	enabled = __vcpu_sys_reg(vcpu, PMCNTENSET_EL0);
	disabled = ~enabled & mask;

	kvm_pmu_enable_counter_mask(vcpu, enabled);
	kvm_pmu_disable_counter_mask(vcpu, disabled);
