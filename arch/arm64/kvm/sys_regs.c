...
static u64 *demux_wp_reg(struct kvm_vcpu *vcpu, const struct sys_reg_desc *rd)
{
...
static bool trap_dbg_wp_reg(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			    const struct sys_reg_desc *rd)
{
	u64 *reg = demux_wp_reg(vcpu, r);
...
static int set_dbg_wp_reg(struct kvm_vcpu *vcpu, const struct sys_reg_desc *rd,
			  u64 val)
{
	u64 *reg = demux_wp_reg(vcpu, r);
...
static int get_dbg_wp_reg(struct kvm_vcpu *vcpu, const struct sys_reg_desc *rd,
			  u64 *val)
{
	u64 *reg = demux_wp_reg(vcpu, r);
...
static u64 reset_dbg_wp_reg(struct kvm_vcpu *vcpu, const struct sys_reg_desc *rd)
{
	u64 *reg = demux_wp_reg(vcpu, r);

	/*
	 * Bail early if we couldn't find storage for the register, the
	 * KVM_BUG_ON() in demux_wp_reg() will prevent this VM from ever
	 * being run.
	 */
	if (!reg)
		return 0;
...
#define DBG_BCR_BVR_WCR_WVR_EL1(n)					\
	{ SYS_DESC(SYS_DBGBVRn_EL1(n)),					\
	  trap_dbg_wp_reg, reset_dbg_wp_reg, 0, 0,			\
	  get_dbg_wp_reg, set_dbg_wp_reg },				\
...
#define DBG_BCR_BVR_WCR_WVR(n)							\
	...
	{ AA32(LO), Op1( 0), CRn( 0), CRm((n)), Op2( 4),			\
	  trap_dbg_wp_reg, NULL, n }, 						\
	/* DBGBCRn */								\
	{ Op1( 0), CRn( 0), CRm((n)), Op2( 5), trap_dbg_wp_reg, NULL, n },	\
	/* DBGWVRn */								\
	{ Op1( 0), CRn( 0), CRm((n)), Op2( 6), trap_dbg_wp_reg, NULL, n },	\
	/* DBGWCRn */								\
	{ Op1( 0), CRn( 0), CRm((n)), Op2( 7), trap_dbg_wp_reg, NULL, n }
