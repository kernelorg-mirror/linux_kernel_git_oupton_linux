...
{
	return IS_ENABLED(CONFIG_KVM_MMU_NOTIFIER_YOUNG_FAST) &&
					...
					unsigned long addr)
{
	return should_walk_secondary_mmu() &&
		mmu_notifier_test_clear_young_fast_only(mm, addr,
							addr + PAGE_SIZE,
							/*clear=*/false);
}
...
{
	return should_walk_secondary_mmu() &&
		mmu_notifier_test_clear_young_fast_only(mm, start, end,
							/*clear=*/true);
}
			       ...
			       pte_t *pte, int *young)
{
	...
	if (notifier_result == -1)
		notifier_result = mmu_notifier_clear_young(
				vma->vm_mm, addr, addr + PAGE_SIZE);
	else
